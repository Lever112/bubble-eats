...
const DEFAULT_SAVE = {
  version: 1,
  endings: {
    end5835: 0, endFull: 0, endFreeze: 0, endSacred: 0,
    endDragonBall: 0, endQuqu: 0, end325: 0, endZero: 0,
    end142: 0, end234: 0, endHigh: 0, endLow: 0,
  },
...
function loadSave() {
  ...
  try {
    ...
    Object.keys(data.endings).forEach(k => {
      if (typeof saved.endings?.[k] === 'number') data.endings[k] = saved.endings[k]
    })
    Object.keys(data.catches).forEach(k => {
      if (typeof saved.catches?.[k] === 'number') data.catches[k] = saved.catches[k]
    })
  } catch (e) {}
  return data
}
  ...
  if (useAudioFallback) {
    try {
      if (type === 'bgm') {
        ...
        bgmEl.loop = true
        bgmEl.volume = 0.4
        bgmEl.play().catch(() => { soundEnabled = false })
      } else {
...
function extractAlpha(key, img, mode = 'stretch') {
  try {
    ...
    alphaCache[key] = { alpha: a, w: img.naturalWidth, h: img.naturalHeight, mode }
  } catch (e) {
    alphaCache[key] = false
  }
}

IMG.goodsList.forEach((g, i) => { g.alphaKey = 'good_' + i })
...
async function preloadAllAssets() {
  ...
  const allImgSrc = [...new Set([
    IMG.gameBg, IMG.startCover,
    IMG.paddle, IMG.paddleGoodFlash, IMG.paddleBombFlash,
    IMG.paddleIceFlash, IMG.paddleXiaolongFlash, IMG.paddleGhostFlash,
    IMG.endZero, IMG.end142, IMG.end234, IMG.end325, IMG.endQuqu,
    IMG.endFreeze, IMG.endSacred, IMG.endDragonBall, IMG.end5835,
    IMG.endHigh, IMG.endFull, IMG.endLow,
    IMG.bomb, IMG.ice, IMG.xiaolong, IMG.ghost,
    ...IMG.goodsList.map(g => g.img),
  ])]

  const total = allImgSrc.reduce((s, src) => s + 1 + (alphaMap[src] ? 1 : 0), 0)
  let done = 0
  function tick() { done++; setProgress(done, total) }

  await Promise.all(allImgSrc.map(imgSrc => new Promise(res => {
    const img = new Image()
    img.crossOrigin = 'anonymous'
    img.onload = () => {
      tick()
      if (alphaMap[imgSrc]) {
        alphaMap[imgSrc].forEach(({ key, mode }) => extractAlpha(key, img, mode))
        tick()
      }
      res()
    }
    img.onerror = () => {
      tick()
      if (alphaMap[imgSrc]) {
        alphaMap[imgSrc].forEach(({ key }) => { alphaCache[key] = false })
        tick()
      }
      res()
    }
...
function pixelCheck(k1, x1, y1, w1, h1, k2, x2, y2, w2, h2) {
  ...
  const oL = Math.max(x1, x2), oR = Math.min(x1 + w1, x2 + w2)
  const oT = Math.max(y1, y2), oB = Math.min(y1 + h1, y2 + h2)
  const step = 2
...
function getScale() {
  ...
  const scaleH = window.innerHeight / (DESIGN_W * 16 / 9)
...
function applyScale() {
  ...
  document.querySelector('.pop-cover').style.width = Math.round(220 * scale) + 'px'
  document.querySelectorAll('.end-img').forEach(el => {
    el.style.width = Math.round(240 * scale) + 'px'
  })
  updatePaddleSize()
  const maxPadX = window.innerWidth - padW
  padX = Math.max(0, Math.min(padX, maxPadX))
  updatePad()
  const vh = window.visualViewport ? window.visualViewport.height : window.innerHeight
  speedScale = Math.max(0.4, Math.min(vh / 800, 1.2))
}
...
window.addEventListener('orientationchange', () => setTimeout(fixViewportHeight, 400))
fixViewportHeight()

// ===================== 全局状态 =====================
let score = 0, life = 3, isPlay = false
let dropList = [], dropSpeed = 3, speedScale = 1
let frameId = null, createTimer = null
let padW = 100, padH = 100, padX = 0
let combo = 0, multiplier = 1.0
let paddleFlashTimer = null
let lastFrameTime = 0
let catchIceCount = 0, catchXiaolongCount = 0
let statBomb = 0, statIce = 0, statXiaolong = 0, statGhost = 0, statFood = 0
...
function createItem() {
  ...
  let attempts = 0
  while (attempts < 15 && dropList.some(ex => Math.abs(ex.x - randomLeft) < minGap)) {
  ...
  dropList.push({
    dom: div, x: randomLeft, y: -itemH, w: itemW, h: itemH,
    speed: (() => {
...
function hitTest(item) {
  ...
  const iL = item.x + shrink, iR = item.x + item.w - shrink
  const iT = item.y + shrink, iB = item.y + item.h - shrink
  if (pR < iL || pL > iR || pB < iT || pT > iB) return false
  return pixelCheck(
    'paddle', pL, pT, padRect.width, pB - pT,
    item.dom.dataset.alphaKey, item.x, item.y, item.w, item.h,
  )
}
  ...
  for (let i = dropList.length - 1; i >= 0; i--) {
    ...
    if (hitTest(item)) {
      ...
      if (type === 'good') {
        ...
        catchIceCount++
        if (catchIceCount >= 15) { gameOver(); return }
      } else if (type === 'xiaolong') {
        playSound('xiaolongSong')
        catchXiaolongCount++
        if (catchXiaolongCount >= 100) { gameOver(); return }
      }
      continue
    }
...
function startCreate() { createTimer = setInterval(createItem, 850) }
function stopCreate()  { if (createTimer) clearInterval(createTimer); createTimer = null }
...
function resetCombo() {
  combo = 0; multiplier = 1.0; updateComboDisplay()
}
...
function isMenuOpen() {
  return !startPop.classList.contains('hide')
...
// ===================== 游戏主流程 =====================
function startGame() {
  startBtn.disabled = restartBtn.disabled = homeBtn.disabled = endingsBtn.disabled = true
  score = 0; life = 3
  ...
  resetCombo(); updatePaddleSize()
  scoreBox.innerText = score; lifeBox.innerText = life
  dropList.forEach(i => i.dom.remove()); dropList = []
  paddle.style.backgroundImage = `url(${IMG.paddle})`
  if (paddleFlashTimer) clearTimeout(paddleFlashTimer)
  paddleFlashTimer = null
  startPop.classList.add('hide'); overPop.classList.add('hide')
  endingsPop.classList.add('hide')
  isPlay = true
  ...
  gameLoop(); startCreate()
  setTimeout(() => {
    startBtn.disabled = restartBtn.disabled = homeBtn.disabled = endingsBtn.disabled = false
  }, 500)
}
...
function returnToMain() {
  ...
  stopCreate(); stopBgm()
  if (paddleFlashTimer) clearTimeout(paddleFlashTimer); paddleFlashTimer = null
  dropList.forEach(i => i.dom.remove()); dropList = []
  paddle.style.backgroundImage = `url(${IMG.paddle})`
  document.getElementById('pauseBtn').style.display = 'none'
  document.getElementById('pauseMask').classList.add('hide')
  overPop.classList.add('hide'); endingsPop.classList.add('hide')
  startPop.classList.remove('hide')
  renderSaveStats(loadSave())
}
...
function gameOver() {
  ...
  cntBomb.innerText = statBomb; cntIce.innerText = statIce
  cntXiaolong.innerText = statXiaolong; cntGhost.innerText = statGhost
  ...
  recordGameResult(endingId, {
    bomb: statBomb, ice: statIce, xiaolong: statXiaolong,
    ghost: statGhost, food: statFood,
  })
...
window.addEventListener('resize', () => { fixViewportHeight(); applyScale() })
...
function resumeGame() {
  if (!isPaused) return
  isPaused = false; isPlay = true
  lastFrameTime = null
  pauseMask.classList.add('hide')
  pauseBtn.innerHTML = '&#9646;&#9646;'
  gameLoop(); startCreate(); playSound('bgm')
}

pauseBtn.addEventListener('click',  () => { if (isPaused) resumeGame(); else pauseGame() })
resumeBtn.addEventListener('click', resumeGame)

window.addEventListener('blur', () => { if (!isPlay || isPaused) return; pauseGame() })
window.addEventListener('focus', () => {
  if (isPaused || isMenuOpen()) return
  if (!isPlay && startPop.classList.contains('hide') && overPop.classList.contains('hide')) {
    isPlay = true; lastFrameTime = null
    gameLoop(); startCreate(); playSound('bgm')
  }
})
